...
export default function SensorDetail() {
  ...
  const fetchSensorData = async (showLoading = true) => {
    ...
    try {
      ...
      console.log(`Found ${sensorsData.length} sensors of type ${sensorType}`);
      ...
      await Promise.all(
        sensorsData.map(async (sensorData) => {
          try {
            ...
            sensorLogs[sensorData.sensorId] = Array.isArray(logsResponse)
              ? logsResponse
              : [];
      ...
      sensorsData.forEach((sensorData) => {
        sensorData.plantIds.forEach((plantId) => {
          ...
          processedGroups.push({
            ...
            plantId: plantId,
            plantName: plantData?.name || `Plant ${plantId.slice(-4)}`,
            sensorId: sensorData.sensorId,
            zone: plantData?.zone || "Unknown Zone",
            value: `${value}${sensorConfig?.unit || ""}`,
            rawValue: value,
            critical: isCritical,
            icon: getPlantIcon(plantData?.zone || "", plantData?.name),
            lastUpdated: latestLog ? new Date(latestLog.timestamp) : new Date(),
